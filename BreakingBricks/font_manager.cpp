/*
file: font_manager.cpp
*/

#include "log.h"
#include "font_manager.h"

FontManager::FontManager(void) {
	static sf::Uint32 defaultCharset[] =
		{
			
		};
		for(unsigned int i = 0; i < 191; ++i) _charset[i] = defaultCharset[i];
}

sf::Font& FontManager::GetFont(const std::string& filename)
{
	fontMapIt_t fontMapIt(_fonts.find(filename));
	if(_fonts.end() == fontMapIt) {
		sf::Font font;
		if(!font.LoadFromFile(filename, 100, _charset)) {
			Log::Instance().Error() << "unable to load font " << filename;
			throw std::exception("unable to load font");
		}
		_fonts[filename] = font;
		fontMapIt = _fonts.find(filename);
	}
	return fontMapIt->second;
}